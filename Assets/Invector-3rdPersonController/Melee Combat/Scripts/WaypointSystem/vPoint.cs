...
namespace Invector {
    ...
    public class vPoint : MonoBehaviour {
        ...
        // EDIT: change if not in mesh set valid false
        public void SetValidIfInNavMesh () {
            NavMeshHit hit;
            isValid = NavMesh.SamplePosition (position, out hit, 0.5f, NavMesh.AllAreas) &&
                Vector3.Distance (position, hit.position) <= 0.4f;
        }
    }
}